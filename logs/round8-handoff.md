...
- [x] **Round 38 (71336d9):** `tests/test_device_id.py` — persisted device-id is
  deterministic + reads back its file (used for the `device` tag in ingest/collect).
  Suite 393 passed, 1 skipped.
- [x] **Round 39 (pending commit):** `tests/test_routes.py` — `classify_existing` applies a
  valid envelope and escalates on an invalid one (fallback). Suite 395 passed, 1 skipped.
