...
- `[done]` **`jarvis delegate` retired (2026-08-07):** the productized offload command was removed. Verified the box's `cline` hosted provider **requires Cline Credits** (`$0.00` balance — the Cline Pass does not translate into usable balance for its Muse Spark model on this box), and delegate-on-local-Ollama is pointless (same box/RAM/model). Jarvis therefore has **zero external dependencies**: task dispatch stays with the Mayor's local `jarvis/agents/*` on Ollama-Lightspeed. If cloud offload is ever wanted, it needs credits or a Pass-covered model — a documented future option, not shipped code.
